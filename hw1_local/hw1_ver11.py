...
def adagrad(x_train,y_train,epoch,lr,size):
	...
	for i in range(epoch):
		...
		ans = x_train.dot(w)
		if ((y_train-ans).transpose().dot(y_train-ans)/y_train.shape[0])**0.5 + lamda*w.transpose().dot(w)/y_train.shape[0]< best:
			best = min(best,((y_train-ans).transpose().dot(y_train-ans)/y_train.shape[0])**0.5 + lamda*w.transpose().dot(w)/y_train.shape[0])
			w_best = w.copy()
		if i % 1000 == 0:
			print("epochs = {} loss = {}".format(i,best))
			np.save('w_ver11.npy',w_best)
...
def linear_re(doc):
	...
	for data_num in range(len(x_train_pre[0])-9):
		c = x_train_pre[2:,data_num:data_num+9].copy()
	...
	adagrad(x_train,y_train,100000,10000,size)
	#print(size)
