...
def adagrad(x_train,y_train,epoch,lr):
	best = 1000000000000
	w = np.zeros([163,1])
	w_pre_grad = 0

	w_best = np.zeros([163,1])

	f = open('weight_his.txt','a')
	...
	for i in range(epoch):
		...
		ans = x_train.dot(w)
		if ((y_train-ans).transpose().dot(y_train-ans)/5751)**0.5 < best:
			best = min(best,((y_train-ans).transpose().dot(y_train-ans)/5751)**0.5)
			w_best = w.copy()
		if i % 10000000 == 0:
			print("epochs = {} loss = {}".format(i,best))
			np.save('w_ver2.npy',w_best)
...
def linear_re(doc):
	...
	for neg in range(len(x_train_pre[9])):
		x_train_pre[9][neg] = max(x_train_pre[9][neg],0)
	...
	adagrad(x_train,y_train,1000000000,0.1)
