...
def adagrad(x_train,y_train,epoch,lr,size,model):
	...
	for i in range(epoch):
		...
		w_grad = -2*x_train_t.dot(y_train-x_train.dot(w)) + 2*0.1*w
		...
		ans = x_train.dot(w)
		if ((y_train-ans).transpose().dot(y_train-ans)/y_train.shape[0])**0.5 + 0.1*w.transpose().dot(w)/y_train.shape[0] < best:
			best = min(best,((y_train-ans).transpose().dot(y_train-ans)/y_train.shape[0])**0.5 + 0.1*w.transpose().dot(w)/y_train.shape[0])
			w_best = w.copy()
		if i % 1000 == 0:
